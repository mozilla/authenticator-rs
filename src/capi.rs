...
#[no_mangle]
pub extern "C" fn rust_u2f_mgr_new() -> *mut FidoManager {
    if let Ok(mgr) = FidoManager::new() {
...
#[no_mangle]
pub unsafe extern "C" fn rust_u2f_mgr_free(mgr: *mut FidoManager) {
    if !mgr.is_null() {
        Box::from_raw(mgr);
    }
}
...
#[no_mangle]
pub unsafe extern "C" fn rust_u2f_mgr_cancel(mgr: *mut FidoManager) {
    if !mgr.is_null() {
        // Ignore return value.
        let _ = (*mgr).cancel();
    }
