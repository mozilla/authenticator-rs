...
use nss_sys::p11::{
    PK11ObjectType, PK11Origin, PK11_CreateContextBySymKey, PK11_Decrypt, PK11_DigestFinal,
    PK11_DigestOp, PK11_Encrypt, PK11_ExtractKeyValue, PK11_GenerateKeyPairWithOpFlags,
    PK11_GetInternalSlot, PK11_GetKeyData, PK11_HashBuf, PK11_ImportSymKey, PK11_PubDeriveWithKDF,
    PK11_ReadRawAttribute, PrivateKey, PublicKey, SECOID_FindOIDByTag, SECOidTag, AES_BLOCK_SIZE,
    CKA_EC_POINT, CKA_ENCRYPT, CKA_SIGN, CKD_NULL, CKD_SHA256_KDF, CKF_DERIVE, CKM_AES_CBC,
    CKM_ECDH1_DERIVE, CKM_EC_KEY_PAIR_GEN, CKM_SHA256_HMAC, CKM_SHA512_HMAC, CKO_PUBLIC_KEY,
    CK_FLAGS, CK_MECHANISM_TYPE, PK11_ATTR_INSENSITIVE, PK11_ATTR_PUBLIC, PK11_ATTR_SESSION,
    SEC_ASN1_OBJECT_ID, SHA256_LENGTH,
};
#[cfg(test)]
use nss_sys::p11::{SECKEYPrivateKeyStr, SECKEYPublicKeyStr};
...
pub static ECDH_P256: Algorithm = Algorithm {
    // SEC_OID_SECG_EC_SECP256R1 is a seperate define outside of the SECOidTag-enum
    // but it maps to PRIME256V1.
...
pub(crate) fn parse_key(_curve: ECDSACurve, x: &[u8], y: &[u8]) -> Result<Vec<u8>> {
    ...
    buf[0] = u8::try_from(EC_POINT_FORM_UNCOMPRESSED)?;
    let mut offset = 1;
...
fn digest(data: &[u8]) -> Result<Vec<u8>> {
    let mut out = vec![0u8; SHA256_LENGTH as usize];
    let data_len = i32::try_from(data.len())?;
    unsafe {
        PK11_HashBuf(
            SECOidTag::SEC_OID_SHA256 as u32,
            out.as_mut_ptr(),
            data.as_ptr(),
            data_len,
        )
...
fn encapsulate_helper(
    ...
    let (x, y) = serialize_key(key.curve, public_key.key_data()?.as_ref())?;
    ...
    let digest = digest(&shared_secret)?;
    Ok(ECDHSecret {
        remote: COSEKey {
            alg,
...
pub(crate) fn test_encapsulate(
    ...
    let private_key =
        unsafe { PrivateKey::from_ptr(my_priv_key.as_ptr() as *mut SECKEYPrivateKeyStr)? };
    let public_key =
        unsafe { PublicKey::from_ptr(my_pub_key.as_ptr() as *mut SECKEYPublicKeyStr)? };
    encapsulate_helper(key, alg, public_key, private_key)
}
...
pub(crate) fn authenticate(token: &[u8], input: &[u8]) -> Result<Vec<u8>> {
    ...
    assert_eq!(digest_len, SHA256_LENGTH as u32);
    Ok(digest)
}
